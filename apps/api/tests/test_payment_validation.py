...
def test_real_stripe_signature_verification(monkeypatch):
    ...
    body = json.dumps({"id":"evt_fixture","object":"event","type":"payment_intent.created","data":{"object":{}}}).encode()
...
@pytest.mark.parametrize("bad_field,value",[("payment_status","unpaid"),("mode","subscription"),("status","open"),("amount_total",2500),("currency","eur"),("id","cs_wrong"),("client_reference_id","wrong-user"),("livemode",True)])
def test_unpaid_or_mismatched_checkout_cannot_credit(monkeypatch,bad_field,value):
    topup_id = str(uuid4())
    data={"id":"cs_fixture","mode":"payment","status":"complete","payment_status":"paid","livemode":False,"payment_intent":"pi_fixture","client_reference_id":"user","amount_total":1000,"currency":"usd","metadata":{"topup_id":topup_id,"user_id":"user"}}
    data[bad_field]=value
    monkeypatch.setattr(payments,"fetch_topup_by_id",AsyncMock(return_value={"id":topup_id,"user_id":"user","stripe_checkout_session_id":"cs_fixture","amount_cents":1000,"currency":"USD"}))
    credit = AsyncMock()
    monkeypatch.setattr(payments,"complete_topup_record",credit)
    with pytest.raises(OpenAIAPIError):
        asyncio.run(payments.handle_checkout_completed(SimpleNamespace(id="evt_fixture",data={"object":data})))
