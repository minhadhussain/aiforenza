      ...
      <section className="border border-white/10 bg-[#050608] p-4">
        {records.length ? (
          <div className="space-y-2">
            <div className="grid gap-3 border-b border-white/10 px-3 py-2 text-xs uppercase tracking-[0.14em] text-[var(--muted)] md:grid-cols-[0.9fr_1fr_0.75fr_0.75fr_0.75fr_0.75fr_0.75fr_0.9fr]">
              <div>DATE</div>
              <div>MODEL</div>
              <div>INPUT</div>
              <div>OUTPUT</div>
            ...
            {records.map((record) => (
              <div key={record.id} className="grid gap-3 border border-white/10 px-3 py-3 text-sm md:grid-cols-[0.9fr_1fr_0.75fr_0.75fr_0.75fr_0.75fr_0.75fr_0.9fr]">
                ...
                <div className="text-[var(--muted)]">{formatUsdFromCents(record.reference_charge_cents)}</div>
                <div className="text-[var(--muted)]">{formatUsdFromCents(record.customer_charge_cents)}</div>
                <div className="text-[var(--muted)]">{formatUsdFromCents(record.customer_savings_cents)}</div>
