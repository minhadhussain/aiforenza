...
declare w public.wallets; p public.topups; tx public.transactions;
begin
  if target_stripe_event_id is null or target_stripe_event_id = '' or target_payment_intent_id is null
     or target_amount_cents is null or target_amount_cents <= 0 or target_currency is distinct from 'USD' then raise exception 'invalid_topup'; end if;
  ...
  if p.id is null or p.user_id <> target_user_id or p.package_value_usd_cents <> target_amount_cents
     or p.currency <> target_currency or p.stripe_payment_intent_id is distinct from coalesce(p.stripe_payment_intent_id,target_payment_intent_id) then
  ...
  update public.wallets set balance_cents = w.balance_cents + p.package_value_usd_cents, updated_at = now() where id = w.id;
  insert into public.transactions(user_id,wallet_id,type,amount_cents,balance_after_cents,reference_id,description)
    values(p.user_id,w.id,'TOPUP',p.package_value_usd_cents,w.balance_cents+p.package_value_usd_cents,'stripe:'||target_checkout_session_id,'Stripe wallet top-up') returning * into tx;
