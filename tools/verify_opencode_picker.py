...
def main(directory, verify_variants=False):
    env = os.environ.copy()
    ...
    try:
        ...
        if verify_variants:
            proc.write("\r")  # Select the sole AI Forenza Astra model, without a prompt.
            read_for(proc, 3)
            observed = set()
            for _ in range(7):
                proc.write("\x14")  # Default Ctrl+T: cycle model reasoning variant.
                footer = read_for(proc, 2)
                observed.update(re.findall(r"\b(low|medium|high|xhigh|max)\b", footer.lower()))
            print(json.dumps({"selected_model": "aiforenza/gpt-6-astra", "observed_effort_labels": sorted(observed), "no_inference_submitted": True}), flush=True)
            if observed != {"low", "medium", "high", "xhigh", "max"}:
...
if __name__ == "__main__":
    ...
    try:
        main(Path(args.directory), args.variants)
