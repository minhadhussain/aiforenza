...
if filename not in {"202609090001_wallet_reservations.sql", "202609090002_payment_credit_safety.sql", "202609090003_catalog_reference_prices.sql"}:
...
try:
    with psycopg2.connect(env["DATABASE_URL"], sslmode="require", connect_timeout=10) as connection:
        with connection.cursor() as cursor:
            cursor.execute("set lock_timeout='5s'; set statement_timeout='30s'")
            cursor.execute("select version from supabase_migrations.schema_migrations where version=%s",(version,))
            if cursor.fetchone():
                print("Migration already applied:", name)
            else:
                cursor.execute(sql.replace("commit;", ""))
                cursor.execute("insert into supabase_migrations.schema_migrations(version,name,statements) values(%s,%s,%s)",(version,name,[sql]))
